...
def getPumpFlux_k12kp_MF(x, y):     # x: kappa_12, y: kp
    net = Network()
...
def getPumpFlux_k12kp_MF_Matrix(X, Y):
    Z = []
    for i in range(len(Y)):
        ...
        for j in range(len(X)):
            ...
            while success == False:
                try:
                    Jpump = getPumpFlux_k12kp_MF(x, y)     # getpop() in the function file ensures solutions such that 0 < probability < 1 are picked
                    print('Jpump=', Jpump)        
...
Z1 = getPumpFlux_k12kp_MF_Matrix(X, Y)
Z2 = getPumpFlux_k12kp_Jiang_Matrix(X, Y)
...
## Mean-field plots ###
mf = ax1.contourf(X, Y, Z1, cmap='Blues_r', levels=color_levels, norm=colors.SymLogNorm(linthresh=10**(-6), linscale=10**(-6)))
ax1.set_xscale('log')
ax1.set_yscale('log')
...
jiang = ax2.contourf(X, Y, Z2, cmap='Blues_r', levels=color_levels, norm=colors.SymLogNorm(linthresh=10**(-6), linscale=10**(-6)))
ax2.set_xscale('log')
ax2.set_yscale('log') 
...
## Color bars ##
cbar = fig.colorbar(jiang, ax=[ax1, ax2], orientation='vertical', format=OOMFormatter(-3, mathText=False))
