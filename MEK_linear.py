...
class Network():
    def __init__(self):
        ...
        self.D = None  # not defined: Adjacency matrix
        self.K = None  # not defined: Rate Matrix
        self.modK = None  # not defined : Modified Rate Matrix that incorporates heme-heme interaction
        ...
        self.reorgE = 1.0    # unit is eV
        self.V = 0.1   # unit is eV
    ...
    def redox_microstate(self, eps: np.array) -> list:
        """
        Assign redox potential to a cofactor at each microstate to incorporate heme-heme interactions.
        Arguments:
            eps[ii][j] :the change of reduction potential of heme j if heme ii is in the oxidized state
        Returns:
            list -- [ [(Redox potential of cof1 at 1st microstate),(Redox potential of cof2 at 1st microstate),...], [(Redox potential of cof1 at 2nd microstate),...], []....]
        """
        # Create a list of cofactors
        cof_list=[]
        for n in range(self.num_cofactor):
            cof_list.append(self.id2cofactor[n])
        # Assign redox potential to a cofactor at each microstate
        redox_microstate_list=[]
        for i in range(self.adj_num_state):
            redox_list=[]
            for j in range(self.num_cofactor):
                redox=cof_list[j].redox[0]
                for ii in range(self.num_cofactor):
                    if ii != j:
                        redox += (1-self.idx2state(self.allow[i])[ii])*eps[ii][j]  # e_j = e_j + Σ_j≠ii=1{(1-Pii)*eps_iij}
                        #print(redox)
                redox_list.append(redox)
                #print(redox_list)
            redox_microstate_list.append(redox_list)

        return redox_microstate_list
    ...
    def evolve_Mod(self, t: float, pop_init: np.array) -> np.array:
        """
        Evolve the population vector with a timestep of t using the modified rate matrix.
        Arguments:
            t {float} -- Time
        Keyword Arguments:
            pop_init {numpy.array} -- Initial population vector (default: None)
    ...
    def ET(self, deltaG: float, R: float, reorgE, V) -> float:
        """
        ...
        """
        # return (2*math.pi/self.hbar)*(V**2)*np.exp(-R)*(1/(math.sqrt(4*math.pi*(1/self.beta)*reorgE)))*np.exp(-self.beta*(deltaG + reorgE)**2/(4*reorgE))
        return (2*math.pi/self.hbar)*(V**2)*(1/(math.sqrt(4*math.pi*(1/self.beta)*reorgE)))*np.exp(-self.beta*(deltaG + reorgE)**2/(4*reorgE))


    def constructAdjacencyMatrix(self):
        """
        Build adjacency matrix by site basis from the adjacency list
        """
...
#       print("list:",self.allow)
    ...
    def constructConservedRateMatrix(self, num_electrons: int):
        """
        Build rate matrix for closed system (total number of electrons are conserved)
        Arguments:
            num_electrons {int}  -- Total number of electrons in a closed system
        """
        ...
        # loop through nonconserved states to remove rate constants
        for i in range(self.num_state):
            if self.idx2state(i)==self.getNonConservedStates(num_electrons)[i]:
            ...
            for j in range(i+1, self.num_cofactor):   # These two "for" loops take care of (upper triangular - diagonal) part of the adjacency matrix
                if self.D[i][j] != 0:  # cofactor i and j are connected!  !=:not equal to
                    ...
                    for donor_state in range(1, cof_i.capacity+1):    #This is correct!!!! Python is weird      #cof.capacity=maximum number of electrons the cofactor can occupy
                        for acceptor_state in range(0, cof_f.capacity):    #This is correct!!!! Python is weird
                            deltaG = cof_i.redox[donor_state-1] - cof_f.redox[acceptor_state]   #This is correct!!!! Python is weird
                            k = self.ET(deltaG, dis, self.reorgE, self.V)
                            self.connectStateRate(i, donor_state, j, acceptor_state, k, deltaG,1)   #Adding the rate constant to rate matrix. The last parameter is 1 because these are all 1-electron transfers!
    ...
    def constructRateMatrix_Mod(self, eps : np.array, reorgE_dict: dict, coupling_dict: dict):
        """
        Build a modified rate matrix. Includes heme-heme interaction.
        """
        redox_microstate = self.redox_microstate(eps)
        # Create a list of cofactors
        cof_list = []
        for n in range(self.num_cofactor):
            cof_list.append(self.id2cofactor[n])
        # Initialize rate matrix that will have the modified rate constants
        self.modK = np.zeros((self.adj_num_state, self.adj_num_state), dtype = float)
        # Determine modified rate constants
        for i in range(self.adj_num_state):
            for j in range(self.adj_num_state):
                if i != j:
                    if self.K[j][i]!=0: # There is transition from microstate i -> j
                        # Search for which cofactor was involved in ET
                        for donorID in range(self.num_cofactor):
                            for acceptorID in range(donorID + 1, self.num_cofactor):
                                if self.idx2state(self.allow[i])[donorID] == 1 and self.idx2state(self.allow[i])[acceptorID] == 0:  # ith microstate: [..,1,0,...]
                                    if self.idx2state(self.allow[j])[donorID] == 0 and self.idx2state(self.allow[j])[acceptorID] == 1:  # jth microstate: [...,0,1,...]
                                        # Donor -> Acceptor. Cofactors involved in ET found!
                                        # Check other electron conservation
                                        I = np.delete(self.idx2state(self.allow[i]), [donorID, acceptorID])
                                        J = np.delete(self.idx2state(self.allow[j]), [donorID, acceptorID])
                                        if np.array_equal(I, J):
                                            dis = self.D[donorID][acceptorID]
                                            deltaG_mod = redox_microstate[self.allow[i]][donorID] - redox_microstate[self.allow[j]][acceptorID]   # Transition of i -> j microstate. Cofactor donor -> acceptor involved
                                            if self.id2cofactor[donorID].name == 'A' and self.id2cofactor[acceptorID].name == 'B':
                                                reorgE = reorgE_dict['λ_AB']
                                                V = coupling_dict['V_AB']
                                                kf = self.ET(deltaG_mod, dis, reorgE, V)
                                                kb = kf * np.exp(self.beta*deltaG_mod)
                                                self.modK[j][i] = kf
                                                self.modK[i][i] -= kf
                                                self.modK[i][j] = kb
                                                self.modK[j][j] -= kb
                                            if self.id2cofactor[donorID].name == 'B' and self.id2cofactor[acceptorID].name == 'C':
                                                ...
                                                self.modK[i][j] = kb
                                                self.modK[j][j] -= kb
                                            if self.id2cofactor[donorID].name == 'C' and self.id2cofactor[acceptorID].name == 'D':
                                                ...
                                                self.modK[j][j] -= kb
                        for reservoir_id, info in self.reservoirInfo.items():
                            name, cofactor, redox_state, num_electron, deltaG, rate = info   # The original deltaG is defined as the deltaG to move an electron from cofactor to reservoir
                            for cofactorID in range(self.num_cofactor):
                                if self.idx2state(self.allow[i])[cofactorID] == 1 and self.idx2state(self.allow[j])[cofactorID] == 0:  # Transition of i -> j microstate. Electron flow cofactor -> reservoir
                                    # Cofactor found!
                                    if self.id2cofactor[cofactorID] == cofactor:
                                        # Reservoir found!
                                        # Check other electron conservation
                                        I = np.delete(self.idx2state(self.allow[i]), [cofactorID])
                                        J = np.delete(self.idx2state(self.allow[j]), [cofactorID])
                                        if np.array_equal(I, J):
                                            deltaG_mod_res = deltaG - cof_list[cofactorID].redox[0] + redox_microstate[self.allow[i]][cofactorID]
                                            kf_res = rate   # This is the cofactor -> reservoir rate
                                            kb_res = rate * np.exp(self.beta*deltaG_mod_res)
                                            self.modK[j][i] = kf_res
                                            self.modK[i][i] -= kf_res
                                            self.modK[i][j] = kb_res
                                            self.modK[j][j] -= kb_res
                                            #print(cofactorID, name, self.idx2state(self.allow[i]), self.idx2state(self.allow[j]), kf_res, kb_res, deltaG_mod_res)
...
#         time_points[i].
    ...
    def listConnectedStates_Mod(self) -> list:
        """
        List the states that are connected. Based on the modified rate matrix
        Returns:
            list -- [i (row number of K), j (column number of K), [ith state], [jth state]]
        """
        #search for rate matrix elements that are nonzero -> search for states that are connected
        ...
        for i in range(self.adj_num_state):     #loop through all the possible states
            for j in range(self.adj_num_state):   #looks through all the possible states
                if (self.modK[i][j]!=0):
                    connectedstates.append([i,j, self.idx2state(self.allow[i]), self.idx2state(self.allow[j]), self.modK[i][j]])   #list up states that are connected
    ...
    def getCofactorRate_Mod(self, cof_i: Cofactor, red_i: int, cof_f: Cofactor, red_f: int, pop: np.array) -> float:
        """
        Calculate the instantaneous forward rate from cof_i to cof_f
        Arguments:
            cof_i {Cofactor} -- Cofactor object for initial cofactor
            ...
            if self.idx2state(self.allow[i])[cof_i_id] == red_i and self.idx2state(self.allow[i])[cof_f_id] == red_f - 1:
                ...
                for j in range(self.adj_num_state):
                    ...
                    if self.idx2state(self.allow[j])[cof_i_id] == red_i - 1 and self.idx2state(self.allow[j])[cof_f_id] == red_f:
                        ...
                        if np.array_equal(I, J):
                            # i and j state found!)
                            flux += self.modK[j][i] * pop[i]      #K is rate matrix, so len(K)=self.num_state
    ...
    def getCofactorFlux_Mod(self, cof_i: Cofactor, red_i: int, cof_f: Cofactor, red_f: int, pop: np.array) -> float:
        """
        Calculate the instantaneous NET flux from initial cofactor(state) to final cofactor(state), by calling getCofactorRate() twice
        Arguments:
            cof_i {Cofactor} -- Cofactor object for initial cofactor
        ...
        Returns:
            float -- Instantaneous net flux
        """
        return self.getCofactorRate_Mod(cof_i, red_i, cof_f, red_f, pop) - self.getCofactorRate_Mod(cof_f, red_f, cof_i, red_i, pop)
    ...
    def getReservoirFlux(self, name: str, pop: np.array) -> float:
        ...
        reverse_rate = rate * np.exp(self.beta*deltaG)    #deltaG varies when electrostatic interaction included
    ...
    def getReservoirFlux_Mod(self, name: str, pop: np.array) -> float:
        """
        ...
        for i in range(self.adj_num_state):
            for j in range(self.adj_num_state):
                if self.idx2state(self.allow[i])[cof_id] == 1 and self.idx2state(self.allow[j])[cof_id] == 0:
                    I = np.delete(self.idx2state(self.allow[i]), [cof_id])
                    J = np.delete(self.idx2state(self.allow[j]), [cof_id])
                    if np.array_equal(I, J):
                        ans += self.modK[j][i]*pop[i] - self.modK[i][j]*pop[j]
